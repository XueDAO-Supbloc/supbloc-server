...
import ModService from "../neo4j/services/mod.service.js";
import QueryService from "../neo4j/services/query.service.js";

import { logger } from "../utils/logger.js";
import { SNAPSHOT_DIR, LOCAL } from "../utils/constants.js";
import { SUPPLIER_LIST } from "../utils/supplierList.js";

import { UploadIpfs } from "../ipfs/ipfs_api.js";
import { push_data, fetch_data } from "../aptos/start_service.js";
...
if (LOCAL) {
  ...
  router.get("/verify", async (req, res) => {
    try {
      ...
      if (stamp > 0) {
        try {
          jsonObj.cid = await fetch_data(stamp);
        }
        catch (err) {
          // logger.error(`Some error occurred when fetching snapshot cid with stamp: ${stamp}`);
          // logger.error(err);
          logger.error(`Recieve cid from neo4j`);
      ...
      if (jsonObj.result === "YES" || jsonObj.cid === "NONE") {
        ...
        if (cid) {
          ...
          try {
            await push_data(stamp, cid);
          }
          catch (err) {
            // logger.error(`Some error occurred when pushing data to blockchain, stamp: ${stamp} and cid: ${cid}`);
            // logger.error(err);
          }
        }
        else
